...
# UCT+13 for NZDT 
def get_nzt_now():
    """Returns the current time in NZDT = UTC + 13 for accurate logging."""
    nz_offset = timezone(timedelta(hours=13))
    return datetime.now(nz_offset)

# Many to Many helper for task tagging
# This junction table connects tasks and tags without repeating data
...
class Status(db.Model):
    """Lookup table for task progress states (e.g., Pending, Completed)."""
    id = db.Column(db.Integer, primary_key=True)
    label = db.Column(db.String(20)) 

# Tracks priority weight for tasks
class Priority(db.Model):
    """Lookup table for sorting tasks by importance (2NF logic)."""
    id = db.Column(db.Integer, primary_key=True)
    level = db.Column(db.String(20))
    weight = db.Column(db.Integer)

class User(db.Model):
    """Primary user account storage with unique constraints for security."""
...
class Subject(db.Model):
    """Main Subject container with cascading deletes for data integrity."""
    ...
    messages = db.relationship('Message', backref='subject', lazy=True, cascade="all, delete-orphan")
    study_sessions = db.relationship('StudySession', backref='subject', lazy=True, cascade="all, delete-orphan")

class Task(db.Model):
    """Task objects linked to subjects and priorities for 2NF sorting."""
    task_id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(50), nullable=False)
    description = db.Column(db.Text)
    due_date = db.Column(db.DateTime)
    
    # Links to status, priority, and parents
...
class Tag(db.Model):
    """Categorical labels for tasks."""
    tag_id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(20), unique=True)

class StudySession(db.Model):
    """Tracks time spent studying a specific subject."""
    id = db.Column(db.Integer, primary_key=True)
    duration = db.Column(db.Integer) # stored in minutes
    timestamp = db.Column(db.DateTime, default=get_nzt_now)
    subject_id = db.Column(db.Integer, db.ForeignKey('subject.subject_id'), nullable=False)

class Message(db.Model):
    """Subject-specific chat messages for collaboration."""
...
class SubjectMember(db.Model):
    """Tracks who belongs to what subject and their invite status (3NF)."""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.user_id'), nullable=False)
    subject_id = db.Column(db.Integer, db.ForeignKey('subject.subject_id'), nullable=False)
...
def lookup_data():
    ...
    if not Priority.query.first():
        db.session.add_all([
            ...
            Priority(level='normal', weight=2), 
            Priority(level='low', weight=3)
        ])
    
    if not Color.query.first():
        db.session.add_all([
            Color(name='blue', hex_code='#007BFF'), 
            Color(name='orange', hex_code='#FF6B4A'),
            Color(name='green', hex_code='#28A745')
        ])
